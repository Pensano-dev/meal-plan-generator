...
function MealPlanGenerator() {
  ...
  const handleFormSubmit = async (event) => {
    ...
    if (!isValid) {
      alert(
        "Invalid input format. Please enter words separated by a comma and a space.",
      );
      return;
    }
    ...
    try {
      ...
      const response = await fetch("http://localhost:5555/api/mealplan", {
        // when we deploy, we'll change this to the deployed backend URL saved as an ENV variable
        method: "POST",
        headers: {
